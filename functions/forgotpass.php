<?php if (substr_count($_SERVER['HTTP_ACCEPT_ENCODING'], 'gzip')) ob_start("ob_gzhandler"); else ob_start();
	...
	do
	{
		$key = substr(str_shuffle("0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"), 0, 50);
		$sql = "SELECT COUNT(*) FROM clients WHERE Keye='$key'";
		$res = mysqli_fetch_array(mysqli_query($conn, $sql));
	}	
	while( $res['COUNT(*)'] != 0 );
	
	$sql = "UPDATE `clients` SET `Keye`='".$key."' WHERE Email='".$em."'";
	...
	{
		$to = $em;
		$subject = "Voltajob - Account password reset";
$message = <<<EOF
      ...
      <tr>
        <td class="innerpadding borderbottom">
          <table width="100%" border="0" cellspacing="0" cellpadding="0">
            ...
            <tr>
              <td class="bodycopy">
                Your account password has been successfully secured.<br>Here is the link to setup a new password:<br>
				<a href="https://www.voltajob.com/forgot.php?keye=$key">www.voltajob.com/forgot.php?keye=$key</a><br>
...
EOF;
		...
		{
			echo "<div style='color: green;' class='well'>Thanks we have sent a mail to ".$to." about the further process.</div>";
		}
	}